...
namespace LibNoise.Operator
{
	...
	public class Terrace : ModuleBase
	{
		...
		public void Add(float input)
		{
			...
			this.m_data.Sort(delegate(float lhs, float rhs) { return lhs.CompareTo(rhs); });
		}
		...
		public void Generate(int steps)
		{
			...
			this.Clear();
			float ts = 2.0f / ((float)steps - 1.0f);
			float cv = -1.0f;
		...
		public override float GetValue(float x, float y, float z)
		{
			...
			float v0 = this.m_data[i0];
			float v1 = this.m_data[i1];
			float a = (smv - v0) / (v1 - v0);
			if (this.m_inverted)
			{
				a = 1.0f - a;
				float t = v0;
